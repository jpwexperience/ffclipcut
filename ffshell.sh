	...
	if (( $subtitleChoice >= 0 )); then
		...
		if [[ $subCmdType =~ (.*)(hdmv|dvd_subtitle)(.*) ]]; then
			...
			if (( $audioChoice == -1 )); then
				...
				cmd="ffmpeg -hide_banner -i \"$dir$base.$ext\" -map 0:a:$audioChoice -map 0:v:$videoChoice -ss $clipStart -t $clipDur -vf subtitles=\"$dir$base.$ext:si=$subtitleChoice\" -map 0:a:$audioChoice -c:a aac -crf $crfIn \"$dir$outputPath\""
			fi
